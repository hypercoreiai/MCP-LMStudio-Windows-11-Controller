...
exports.applyTsd = applyTsd;
const ajv_1 = __importDefault(require("ajv"));
const hooks_1 = require("../hooks");
const errors_1 = require("../errors");
const logger_1 = require("../logger");
const log = (0, logger_1.scopedLogger)('core/tsd/applier');
...
function checkRateLimit(tsd) {
    ...
    if (now - state.lastCleanup > CLEANUP_INTERVAL_MS) {
        ...
        if (rateLimitStates.size > MAX_RATE_LIMIT_ENTRIES) {
            log.warn({ size: rateLimitStates.size, max: MAX_RATE_LIMIT_ENTRIES }, 'Rate limit cache exceeded max size, clearing oldest entries');
    ...
    if (state.timestamps.length >= maxTotal) {
        throw new errors_1.RateLimitError(tsd.toolName);
    }
    // Record this call
...
// Elevation check
// ---------------------------------------------------------------------------
async function checkElevation(tsd, sessionConfig) {
    if (!tsd.requiresElevation)
        return;
    ...
    if (process.platform === 'win32') {
        ...
        try {
            ...
            // Not elevated — throw
            throw new errors_1.ElevationError(tsd.toolName);
        }
    }
...
// Input validation
// ---------------------------------------------------------------------------
function validateInput(tsd, args) {
    if (!tsd.inputValidation)
        return;
    const valid = ajv.validate(tsd.inputValidation, args);
    if (!valid) {
        throw new errors_1.ValidationError(tsd.toolName, ajv.errors ?? []);
    }
}
// ---------------------------------------------------------------------------
// Backoff helper
// ---------------------------------------------------------------------------
function computeDelay(tsd, attempt) {
...
// ---------------------------------------------------------------------------
async function executeWithTimeout(executeFn, timeoutMs, toolName) {
    if (!timeoutMs)
        return executeFn();
    let timeoutHandle;
    try {
        return await Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutHandle = setTimeout(() => {
                    reject(new errors_1.TimeoutError(toolName, timeoutMs));
                }, timeoutMs);
            })
        ]);
    }
 ...
 */
async function applyTsd(invocation, tsd, sessionConfig, executeFn, fallbackFn) {
    ...
    if (tsd.preHook) {
        args = await (0, hooks_1.runPreHook)(tsd.preHook, {
            toolName,
            args,
            sessionConfig
        });
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        try {
            result = await executeWithTimeout(() => executeFn(toolName, args), tsd.timeoutMs, toolName);
    ...
    if (!result.success && tsd.fallbackTool && fallbackFn) {
        ...
        try {
            ...
            log.error({ fallback: tsd.fallbackTool, error: e.message }, 'Fallback tool also failed');
            // Keep the original failure result
        }
    }
    // 7. Post-hook
    if (tsd.postHook) {
        result = await (0, hooks_1.runPostHook)(tsd.postHook, {
            toolName,
            args,
            result,
            sessionConfig
