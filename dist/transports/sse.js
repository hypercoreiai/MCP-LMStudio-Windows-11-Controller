...
const log = (0, logger_1.scopedLogger)('transports/sse');
...
function createSseTransport(registry, parser, _sessionConfig) {
    ...
    app.get('/sse/connect', (req, res) => {
        try {
            ...
            log.error({ error: e.message }, 'Error establishing SSE connection');
            if (!res.headersSent) {
    ...
    app.post('/sse/tools/call', async (req, res) => {
        ...
        try {
            ...
            const modelOutput = req.body.model_output;
            ...
            (async () => {
                try {
                    ...
                    log.error({ callId, error: e.message }, 'SSE tool call failed');
                ...
                log.error({ callId: req.body.call_id, error: e.message, stack: e.stack }, 'Unhandled error in SSE async processing');
            });
        }
        catch (e) {
            // Synchronous error boundary
            log.error({ error: e.message, stack: e.stack }, 'Synchronous error in SSE tools/call handler');
            if (!res.headersSent) {
