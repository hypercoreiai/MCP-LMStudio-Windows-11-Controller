...
const log = (0, logger_1.scopedLogger)('tools/window_manager');
// ---------------------------------------------------------------------------
// PowerShell helpers
// ---------------------------------------------------------------------------
function ps(script, timeoutMs = 10000) {
    try {
        ...
        throw new errors_1.ExecutionError('window_manager', stderr || e.message);
    }
}
// ---------------------------------------------------------------------------
// Internal dispatch
// ---------------------------------------------------------------------------
async function handleWindowList(_args) {
    const script = `
...
async function handleWindowFocus(args) {
    const pid = args.pid;
    const script = `
...
async function handleWindowMove(args) {
    const pid = args.pid;
    const x = args.x;
    const y = args.y;
    const script = `
...
async function handleWindowResize(args) {
    const pid = args.pid;
...
async function handleWindowMinimize(args) {
    const pid = args.pid;
    // SW_MINIMIZE = 9
    const script = `
    $proc = Get-Process -Id ${pid} -ErrorAction Stop;
...
async function handleWindowMaximize(args) {
    const pid = args.pid;
    // SW_MAXIMIZE = 3
    const script = `
    $proc = Get-Process -Id ${pid} -ErrorAction Stop;
...
async function handleWindowRestore(args) {
    const pid = args.pid;
...
async function handleWindowClose(args) {
    const pid = args.pid;
    // WM_CLOSE = 0x0010
    const script = `
    $proc = Get-Process -Id ${pid} -ErrorAction Stop;
...
async function handleWindowSnap(args) {
    const pid = args.pid;
    const slot = args.slot; // "left", "right", "top", "bottom", "topLeft", "topRight", "bottomLeft", "bottomRight"
...
const windowManager = {
    ...
    async execute(toolName, args) {
        ...
        switch (toolName) {
            ...
            default:
                throw new errors_1.ExecutionError('window_manager', `Unknown tool: ${toolName}`);
        }
    }
};
// Self-register
registry_1.registry.register(windowManager);
