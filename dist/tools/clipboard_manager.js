...
const log = (0, logger_1.scopedLogger)('tools/clipboard_manager');
// ---------------------------------------------------------------------------
// PowerShell helper
// ---------------------------------------------------------------------------
function ps(script, timeoutMs = 5000) {
    try {
        ...
        throw new errors_1.ExecutionError('clipboard_manager', e.stderr?.toString() || e.message);
    }
}
// ---------------------------------------------------------------------------
// Handlers
// ---------------------------------------------------------------------------
async function handleClipboardGet(args) {
...
async function handleClipboardSet(args) {
    ...
    throw new errors_1.ExecutionError('clipboard_manager', 'Either "text" or "imageBase64" must be provided');
}
async function handleClipboardClear(_args) {
    const script = `
    Add-Type -AssemblyName System.Windows.Forms;
...
const clipboardManager = {
    ...
    async execute(toolName, args) {
        ...
        switch (toolName) {
            ...
            default:
                throw new errors_1.ExecutionError('clipboard_manager', `Unknown tool: ${toolName}`);
        }
    }
};
registry_1.registry.register(clipboardManager);
exports.default = clipboardManager;
//# sourceMappingURL=clipboard_manager.js.map