...
const log = (0, logger_1.scopedLogger)('tools/input_handler');
// ---------------------------------------------------------------------------
...
function ps(script, timeoutMs = 5000) {
    try {
        ...
        throw new errors_1.ExecutionError('input_handler', e.stderr?.toString() || e.message);
    }
}
// ---------------------------------------------------------------------------
// Handlers
// ---------------------------------------------------------------------------
async function handleInputType(args) {
    const text = args.text;
    const delayMs = args.delayMs ?? 0; // optional per-character delay
    if (nativeAddon) {
...
async function handleInputKey(args) {
    const key = args.key;
...
async function handleInputMouseClick(args) {
    const x = args.x;
    const y = args.y;
...
async function handleInputMouseDrag(args) {
...
async function handleInputMouseScroll(args) {
    const x = args.x;
    const y = args.y;
    const ticks = args.ticks; // positive = up, negative = down
...
async function handleInputHotkey(args) {
    const combo = args.combo; // e.g. "Ctrl+C", "Alt+F4", "Ctrl+Shift+Esc"
...
async function handleInputMultiClick(args) {
    const clicks = args.clicks;
    if (!clicks || !Array.isArray(clicks)) {
        throw new errors_1.ExecutionError('input_handler', 'Clicks must be an array');
    }
...
async function handleInputMultiType(args) {
    const types = args.types;
    if (!types || !Array.isArray(types)) {
        throw new errors_1.ExecutionError('input_handler', 'Types must be an array');
    }
...
async function handleInputScrapeUI(args) {
...
const inputHandler = {
    ...
    async execute(toolName, args) {
        ...
        switch (toolName) {
            ...
            default:
                throw new errors_1.ExecutionError('input_handler', `Unknown tool: ${toolName}`);
        }
    }
};
registry_1.registry.register(inputHandler);
