...
const log = (0, logger_1.scopedLogger)('tools/system_info');
// ---------------------------------------------------------------------------
// PowerShell helper
// ---------------------------------------------------------------------------
function ps(script, timeoutMs = 10000) {
    try {
        ...
        throw new errors_1.ExecutionError('system_info', e.stderr?.toString() || e.message);
    }
}
// ---------------------------------------------------------------------------
// Handlers
// ---------------------------------------------------------------------------
async function handleSystemInfo(_args) {
    const script = `
  ...
  `;
    try {
        const raw = ps(script);
        if (!raw || raw.trim() === '') {
            throw new errors_1.ExecutionError('system_info', 'PowerShell returned empty output');
        }
        return { success: true, data: JSON.parse(raw), durationMs: 0 };
    }
    catch (e) {
        if (e instanceof errors_1.ExecutionError)
            throw e;
        throw new errors_1.ExecutionError('system_info', `Failed to parse system info: ${e.message}`);
    }
}
async function handleSystemCpu(_args) {
    const script = `
  ...
  `;
    try {
        const raw = ps(script);
        if (!raw || raw.trim() === '') {
            throw new errors_1.ExecutionError('system_info', 'PowerShell returned empty output for CPU info');
        }
        return { success: true, data: JSON.parse(raw), durationMs: 0 };
    }
    catch (e) {
        if (e instanceof errors_1.ExecutionError)
            throw e;
        throw new errors_1.ExecutionError('system_info', `Failed to parse CPU info: ${e.message}`);
    }
}
async function handleSystemMemory(_args) {
    const script = `
    $os = Get-CimInstance -ClassName Win32_OperatingSystem;
  ...
  `;
    try {
        const raw = ps(script);
        if (!raw || raw.trim() === '') {
            throw new errors_1.ExecutionError('system_info', 'PowerShell returned empty output for memory info');
        }
        return { success: true, data: JSON.parse(raw), durationMs: 0 };
    }
    catch (e) {
        if (e instanceof errors_1.ExecutionError)
            throw e;
        throw new errors_1.ExecutionError('system_info', `Failed to parse memory info: ${e.message}`);
    }
}
async function handleSystemDisk(_args) {
    const script = `
...
async function handleSystemNetwork(_args) {
    const script = `
  ...
  `;
    ...
    if (!raw || raw.trim() === '') {
        throw new errors_1.ExecutionError('system_info', 'PowerShell returned empty output for network info');
    }
    try {
        return { success: true, data: JSON.parse(raw), durationMs: 0 };
    }
    catch (e) {
        throw new errors_1.ExecutionError('system_info', `Failed to parse network info: ${e.message}`);
    }
}
async function handleSystemBattery(_args) {
    const script = `
  ...
  `;
    try {
        const raw = ps(script);
        if (!raw || raw.trim() === '') {
            throw new errors_1.ExecutionError('system_info', 'PowerShell returned empty output for battery info');
        }
        return { success: true, data: JSON.parse(raw), durationMs: 0 };
    }
    catch (e) {
        if (e instanceof errors_1.ExecutionError)
            throw e;
        throw new errors_1.ExecutionError('system_info', `Failed to parse battery info: ${e.message}`);
    }
}
async function handleSystemServices(args) {
    const filter = args.filter; // 'running' | 'stopped' | 'all'
    const namePattern = args.namePattern; // wildcard pattern like "Win*"
    let whereClause = '';
    if (filter === 'running')
        whereClause += ' | Where-Object { $_.State -eq "Running" }';
    else if (filter === 'stopped')
        whereClause += ' | Where-Object { $_.State -eq "Stopped" }';
    let nameClause = '';
    if (namePattern)
        nameClause = `-Name "${namePattern}"`;
    const script = `
...
const systemInfo = {
    ...
    async execute(toolName, args) {
        ...
        switch (toolName) {
            ...
            default:
                throw new errors_1.ExecutionError('system_info', `Unknown tool: ${toolName}`);
        }
    }
};
registry_1.registry.register(systemInfo);
