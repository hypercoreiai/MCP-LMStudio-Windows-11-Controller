...
const log = (0, logger_1.scopedLogger)('tools/agent_orchestrator');
// ---------------------------------------------------------------------------
// LLM Integration
// ---------------------------------------------------------------------------
...
async function loadQwenToken() {
    try {
        await ensureMemoryDir();
        const filePath = (0, path_1.join)(MEMORY_DIR, `${QWEN_TOKEN_KEY}.json`);
...
async function saveQwenToken(token) {
    try {
        await ensureMemoryDir();
        const filePath = (0, path_1.join)(MEMORY_DIR, `${QWEN_TOKEN_KEY}.json`);
        await fs_1.promises.writeFile(filePath, JSON.stringify(token, null, 2));
    }
    catch (e) {
...
async function loadPendingAuth() {
    try {
        await ensureMemoryDir();
        const filePath = (0, path_1.join)(MEMORY_DIR, `${QWEN_PENDING_AUTH_KEY}.json`);
        const data = await fs_1.promises.readFile(filePath, 'utf-8');
        return JSON.parse(data);
    }
    catch {
...
async function savePendingAuth(pending) {
    try {
        await ensureMemoryDir();
        const filePath = (0, path_1.join)(MEMORY_DIR, `${QWEN_PENDING_AUTH_KEY}.json`);
        await fs_1.promises.writeFile(filePath, JSON.stringify(pending, null, 2));
    }
    catch (e) {
...
async function requestDeviceCode(challenge) {
    ...
    if (!response.ok) {
        ...
        throw new errors_1.ExecutionError('agent_orchestrator', `Qwen device authorization failed: ${text || response.statusText}`);
    }
    const payload = await response.json();
    if (!payload.device_code || !payload.user_code || !payload.verification_uri) {
        throw new errors_1.ExecutionError('agent_orchestrator', 'Qwen device authorization returned incomplete payload');
    }
    return payload;
}
async function pollDeviceToken(deviceCode, verifier) {
    ...
    if (!response.ok) {
        ...
        throw new errors_1.ExecutionError('agent_orchestrator', `Qwen OAuth failed: ${payload.error_description || payload.error || response.statusText}`);
    }
    const tokenPayload = await response.json();
    if (!tokenPayload.access_token || !tokenPayload.refresh_token || !tokenPayload.expires_in) {
        throw new errors_1.ExecutionError('agent_orchestrator', 'Qwen OAuth returned incomplete token payload');
    }
    return {
...
async function getValidQwenToken() {
    ...
    if (!token) {
        ...
        if (token) {
            ...
            throw new errors_1.ExecutionError('agent_orchestrator', `Qwen OAuth pending. Please:\n` +
    ...
    if (token.expires < Date.now() + 5 * 60 * 1000) {
        ...
        if (refreshedToken) {
            ...
            await saveQwenToken({}); // Clear invalid token
...
async function callLLM(messages, tools) {
    if (USE_QWEN) {
        ...
        if (!token) {
            throw new errors_1.ExecutionError('agent_orchestrator', 'Failed to obtain Qwen OAuth token');
        }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new errors_1.ExecutionError('agent_orchestrator', `Qwen API error: ${response.status} ${response.statusText} - ${errorText}`);
                }
                ...
                if (error.name === 'AbortError') {
                    ...
                    throw new errors_1.ExecutionError('agent_orchestrator', 'Qwen API request timed out after 30 seconds (all retries exhausted)');
                }
        ...
        throw new errors_1.ExecutionError('agent_orchestrator', `Qwen API network error after 3 attempts: ${lastError?.message}`);
    }
    else {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new errors_1.ExecutionError('agent_orchestrator', `LLM API error: ${response.status} ${response.statusText} - ${errorText}`);
                }
                const data = await response.json();
                const choice = data.choices[0];
                const content = choice.message.content || '';
        ...
        throw new errors_1.ExecutionError('agent_orchestrator', `LLM API unavailable after 3 attempts: ${errorMsg}. Ensure either: 1) Qwen OAuth is configured (USE_QWEN=true), or 2) LM Studio is running at ${OPENAI_BASE_URL}`);
    }
}
// ---------------------------------------------------------------------------
// Handler
...
// Handler
// ---------------------------------------------------------------------------
async function handleAgentExecuteQuery(args) {
    const query = args.query.toLowerCase();
    if (!query) {
        throw new errors_1.ExecutionError('agent_orchestrator', 'Query is required');
    }
    ...
    const allTools = registry_1.registry.list();
    ...
    const otherTools = allTools.filter(t => !t.name.startsWith('firefox.') &&
...
const agentOrchestrator = {
    ...
    async execute(toolName, args) {
        ...
        switch (toolName) {
            ...
            default:
                throw new errors_1.ExecutionError('agent_orchestrator', `Unknown tool: ${toolName}`);
        }
    }
};
// Self-register
registry_1.registry.register(agentOrchestrator);
