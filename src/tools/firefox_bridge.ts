...
const log = scopedLogger('tools/firefox_bridge');
...
function ps(script: string, timeoutMs = 15000): string {
  try {
    ...
    throw new ExecutionError('firefox_bridge', stderr || e.message);
  }
}
...
async function handleFirefoxExecuteJS(args: Record<string, unknown>): Promise<ToolResult> {
  const code = args.code as string;
  if (!code) throw new ExecutionError('firefox_bridge', 'JavaScript code is required');
...
async function handleFirefoxGetElements(args: Record<string, unknown>): Promise<ToolResult> {
  const selector = args.selector as string || 'input, button, a, [role="button"], [contenteditable="true"]';
...
const firefoxBridge: ToolModule = {
  name: 'firefox_bridge',
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    switch (toolName) {
      ...
      default:
        throw new ExecutionError('firefox_bridge', `Unknown tool: ${toolName}`);
    }
  }
};

// Self-register
registry.register(firefoxBridge);
