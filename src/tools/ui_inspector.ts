...
const log = scopedLogger('tools/ui_inspector');
...
async function handleUIListElements(args: Record<string, unknown>): Promise<ToolResult> {
...
async function handleUIGetElement(args: Record<string, unknown>): Promise<ToolResult> {
...
async function handleUIClickElement(args: Record<string, unknown>): Promise<ToolResult> {
  const automationId = args.automationId as string;
  const name = args.name as string;
  const className = args.className as string;
  const rootHandle = args.rootHandle as number | undefined;
...
async function handleUITypeIntoElement(args: Record<string, unknown>): Promise<ToolResult> {
  const automationId = args.automationId as string;
  const name = args.name as string;
  const className = args.className as string;
...
const uiInspector: ToolModule = {
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    switch (toolName) {
      ...
      default:
        throw new ExecutionError('ui_inspector', `Unknown tool: ${toolName}`);
    }
  }
};
