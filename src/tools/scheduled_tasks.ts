...
const log = scopedLogger('tools/scheduled_tasks');
...
function ps(script: string, timeoutMs = 15000): string {
  try {
    ...
    throw new ExecutionError('scheduled_tasks', e.stderr?.toString() || e.message);
  }
}
...
async function handleTaskList(args: Record<string, unknown>): Promise<ToolResult> {
  const folder = (args.folder as string) ?? '\\'; // root folder by default
...
async function handleTaskCreate(args: Record<string, unknown>): Promise<ToolResult> {
  const taskName      = args.taskName as string;
  const taskPath      = (args.taskPath as string) ?? '\\';
  const executable    = args.executable as string;
  const taskArgs      = (args.arguments as string) ?? '';
  const triggerType   = args.triggerType as string; // 'schedule' | 'atLogon' | 'atStartup' | 'onEvent'
  const schedule      = args.schedule as string | undefined; // cron-like or specific time for 'schedule'
  const user          = (args.user as string) ?? 'SYSTEM';
  const runLevel      = (args.runLevel as string) ?? 'LeastPrivilege'; // LeastPrivilege | HighestPrivilege
  ...
  switch (triggerType) {
    ...
    case 'schedule':
      if (!schedule) throw new ExecutionError('scheduled_tasks', 'schedule is required when triggerType is "schedule"');
      ...
      if (parts[0] === 'daily') {
        ...
        throw new ExecutionError('scheduled_tasks', `Unsupported schedule format: "${schedule}". Use: "daily HH:MM", "weekly DayName HH:MM", or "once YYYY-MM-DD HH:MM"`);
      }
      break;

    case 'onEvent':
      // args should include eventLogName and eventId
      const logName = (args.eventLogName as string) ?? 'System';
    ...
    default:
      throw new ExecutionError('scheduled_tasks', `Unknown triggerType: "${triggerType}". Use: schedule, atLogon, atStartup, onEvent`);
  }
...
async function handleTaskDelete(args: Record<string, unknown>): Promise<ToolResult> {
  const taskName = args.taskName as string;
  const taskPath = (args.taskPath as string) ?? '\\';
...
async function handleTaskEnable(args: Record<string, unknown>): Promise<ToolResult> {
  const taskName = args.taskName as string;
  const taskPath = (args.taskPath as string) ?? '\\';
...
async function handleTaskDisable(args: Record<string, unknown>): Promise<ToolResult> {
  const taskName = args.taskName as string;
  const taskPath = (args.taskPath as string) ?? '\\';
...
async function handleTaskRunNow(args: Record<string, unknown>): Promise<ToolResult> {
  const taskName = args.taskName as string;
  const taskPath = (args.taskPath as string) ?? '\\';
...
const scheduledTasks: ToolModule = {
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    switch (toolName) {
      ...
      default:
        throw new ExecutionError('scheduled_tasks', `Unknown tool: ${toolName}`);
    }
  }
};

registry.register(scheduledTasks);
