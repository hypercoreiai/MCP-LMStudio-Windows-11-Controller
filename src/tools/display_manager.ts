...
const log = scopedLogger('tools/display_manager');
...
function ps(script: string, timeoutMs = 10000): string {
  try {
    ...
    throw new ExecutionError('display_manager', e.stderr?.toString() || e.message);
  }
}
...
async function handleDisplayList(_args: Record<string, unknown>): Promise<ToolResult> {
  const script = `
...
async function handleDisplayScreenshot(args: Record<string, unknown>): Promise<ToolResult> {
  const monitorIndex = args.monitorIndex as number | undefined;
...
async function handleDisplayScreenshotAnnotated(args: Record<string, unknown>): Promise<ToolResult> {
  const monitorIndex = args.monitorIndex as number | undefined;
  const region       = args.region as { x: number; y: number; width: number; height: number } | undefined;
  const elements     = args.elements as Array<{ x: number; y: number; width: number; height: number; name?: string }> | undefined;
...
async function handleDisplaySetResolution(args: Record<string, unknown>): Promise<ToolResult> {
  const deviceName = args.deviceName as string;
  const width      = args.width as number;
  const height     = args.height as number;
  const refreshRate = (args.refreshRate as number) ?? 60;
...
async function handleDisplaySetDpi(args: Record<string, unknown>): Promise<ToolResult> {
  const deviceName = args.deviceName as string;
  const dpi        = args.dpi as number;
...
async function handleDisplaySetBrightness(args: Record<string, unknown>): Promise<ToolResult> {
  const level = args.level as number; // 0–100
  ...
  try {
    ...
    return {
      success: false,
      error: { code: 'BRIGHTNESS_UNAVAILABLE', message: (e as Error).message },
      durationMs: 0
    };
  }
}
...
const displayManager: ToolModule = {
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    ...
    switch (toolName) {
      ...
      default:
        throw new ExecutionError('display_manager', `Unknown tool: ${toolName}`);
    }
  }
};
