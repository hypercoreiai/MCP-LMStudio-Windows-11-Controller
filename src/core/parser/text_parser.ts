...
const log = scopedLogger('core/parser/text_parser');
...
function tryFencedJson(rawOutput: string): ToolInvocation | null {
 ...
 */
function tryBareJson(rawOutput: string): ToolInvocation | null {
  ...
  if (end === -1) return null; // unclosed brace
 ...
 */
function tryKnownToolHeuristic(
  rawOutput: string,
 ...
 */
function tryParseToolPayload(json: string, rawOutput: string): ToolInvocation | null {
  ...
  if (typeof obj.name === 'string') {
    toolName = obj.name;
    toolArgs = (obj.arguments as Record<string, unknown>) ?? (obj.args as Record<string, unknown>) ?? {};
...
export interface TextParserResult {
  invocation: ToolInvocation | null;   // null if no tool call was detected
  rawText: string;                     // the original output, always returned
}
...
export function parseTextToolCall(
  ...
  if (fenced) return { invocation: fenced, rawText: rawOutput };

  // Strategy 2
  const bare = tryBareJson(rawOutput);
  if (bare) return { invocation: bare, rawText: rawOutput };

  // Strategy 3
  if (options.knownToolNames.length > 0) {
    const heuristic = tryKnownToolHeuristic(rawOutput, options.knownToolNames);
    if (heuristic) return { invocation: heuristic, rawText: rawOutput };
  }
